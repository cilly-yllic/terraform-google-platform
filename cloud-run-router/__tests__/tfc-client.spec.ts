...
describe('parseRunMessage', () => {
  ...
  it('defaults labels to [] when absent (forward compat)', () => {
    const result = parseRunMessage('{"service":"svc","environments":["dev-001"],"source_repo":"o/r"}')
    expect(result).toEqual({
      service: 'svc',
  ...
  it('accepts an explicitly empty labels array', () => {
    const result = parseRunMessage('{"service":"svc","environments":["prd-001"],"labels":[],"source_repo":"o/r"}')
  ...
  it('returns null when service is an empty string', () => {
    expect(parseRunMessage('{"service":"","environments":["dev-001"],"source_repo":"o/r"}')).toBeNull()
  })
  ...
  it('returns null when environments contains non-string entries', () => {
    expect(parseRunMessage('{"service":"svc","environments":["dev-001",42],"source_repo":"o/r"}')).toBeNull()
  })

  it('returns null when labels contains non-string entries', () => {
    expect(parseRunMessage('{"service":"svc","environments":["dev-001"],"labels":[1],"source_repo":"o/r"}')).toBeNull()
  })
