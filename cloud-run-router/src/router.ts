 ...
 */
const resolveMetadata = async (notification: TfcNotification, service: string, config: Config): Promise<TfcRunMeta> => {
  if (config.metadataSource === 'run_message' || config.metadataSource === 'both') {
    ...
    if (config.metadataSource === 'run_message') {
      throw new Error(`run_message does not contain valid metadata JSON: "${notification.run_message}"`)
    }
  }

  if (config.metadataSource === 'run_variables' || config.metadataSource === 'both') {
    if (!config.tfcApiToken) {
      throw new Error('TFC_API_TOKEN is required when metadata_source is run_variables or both (fallback)')
    }
 ...
 * このオブジェクトで明示する)。
 */
export const handleNotification = async (notification: TfcNotification, config: Config): Promise<HandleResult> => {
  ...
  const latestStatus = notification.notifications[notification.notifications.length - 1]?.run_status
