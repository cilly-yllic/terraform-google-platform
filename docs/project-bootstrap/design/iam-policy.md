...
>
> いずれも **bootstrap の root folder (`FOLDER_ID`) の配下**に置くこと (Factory SA の `projectCreator` / `projectIamAdmin` が継承で届く)。root 外 / org 直下に作るとサービス project に grant が届かず、per-env SA への owner 付与が失敗する。service 用 folder は事前作成し ID を settings.yml に書く (TF では作らない = `roles/resourcemanager.folderCreator` 不要)。
