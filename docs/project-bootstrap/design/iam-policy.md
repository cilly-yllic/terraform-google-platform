...
| Role | 理由 | スコープの考え方 |
|------|------|------------------|
| `roles/resourcemanager.projectCreator` | サービス用 GCP Project を作成する | **folder 推奨**。`BOOTSTRAP_FOLDER_ID` 設定時は folder に付与し、作成可能範囲をその folder 内に限定。`ORGANIZATION_ID` のみの場合は org 全体 (floor) |
| `roles/resourcemanager.projectIamAdmin` | 作成した Project の per-env SA に owner を付与する | 同上。folder 限定が望ましい |

> **folder vs org**: `projectCreator` は性質上 org か folder にしか付与できない (まだ存在しない project には付与できないため)。folder を用意できる環境では folder を指定し、Factory SA が触れる範囲をその folder 内に封じ込めること。bootstrap は `BOOTSTRAP_FOLDER_NAME` (display name) から folder を find-or-create して `BOOTSTRAP_FOLDER_ID` を解決できる (`scripts/bootstrap/_commands/ensure_folder.sh`)。folder mode では folder が org に優先する (`grant_iam.sh`)。folder 無し (org 直下) でも動くが、Factory SA の到達範囲が org 全体になる点を許容する判断が必要。いずれの場合も「Factory SA を *誰が* 使えるか」は §2 の `terraform_workspace_kind=factory` で factory workspace のみに絞られている。
>
> **重要 (folder mode の linkage)**: folder スコープの grant はその folder 内 (配下含む) の project にしか効かない。よって**サービス project も bootstrap の root folder 配下に作成する**必要がある。配置先 folder の優先順位は:
>
> 1. service repo の `settings.yml` の **`folder_id`** (service ごとに事前作成した folder。推奨)
> 2. `dispatch-project-bootstrap` action の `parent_folder_id` input (共通 fallback)
> 3. `parent_organization_id` input (org 直下運用)
>
> いずれも **bootstrap の root folder (`BOOTSTRAP_FOLDER_ID`) の配下**に置くこと (Factory SA の `projectCreator` / `projectIamAdmin` が継承で届く)。root 外 / org 直下に作るとサービス project に grant が届かず、per-env SA への owner 付与が失敗する。service 用 folder は事前作成し ID を settings.yml に書く (TF では作らない = `roles/resourcemanager.folderCreator` 不要)。
