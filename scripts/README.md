...
| **deploy SA** (`cloud-run-router-deploy`) | GitHub Actions が impersonate する identity。project 全体に `roles/run.admin` (deploy + `--allow-unauthenticated` 用の `setIamPolicy`) / `roles/artifactregistry.writer` / `roles/cloudbuild.builds.editor` / `roles/storage.admin` / `roles/secretmanager.secretVersionAdder` を付与。**runtime SA リソース限定**で `roles/iam.serviceAccountUser` (`--service-account=<runtime>` 用) + `roles/iam.serviceAccountTokenCreator` (token 発行) を付与 (tokenCreator を project レベルにしないことで、deploy SA が infra 内の他 SA を impersonate する経路を塞ぐ) |
| **GitHub WIF Provider** | 既存 WIF Pool に追加。issuer = `https://token.actions.githubusercontent.com`、attribute condition = `assertion.repository_owner == "${GITHUB_OWNER}"`（**org 単位**ゲート。`GITHUB_OWNER` 未指定時は `GITHUB_REPOSITORY` の owner）。個々の repo 制限は各 SA の WIF binding (`attribute.repository/{owner}/{repo}`) で行う設計 — これにより各サービス repo の GitHub Actions が自分の firebase project の `ci_service_account` を impersonate して deploy できる |
