...
#   1. FOLDER_ID 直接指定        → folder mode (FOLDER_NAME が同時に在っても無視)
#   2. FOLDER_NAME (+ ORG)       → folder mode (ensure_folder が find-or-create で
#                                 FOLDER_ID を解決し .env に書き戻す)
...
#
# 注: FOLDER_NAME 解決後は ensure_folder が FOLDER_ID を .env に書き戻すため、
# 再実行時は FOLDER_ID と FOLDER_NAME が「両方」セットされた状態になる。これは
# 正常系 (FOLDER_ID 優先で FOLDER_NAME は無視) であり、エラーにしない。
check_org_folder() {
  ...
  if [[ -n "${folder}" ]]; then
    ...
    if [[ -n "${folder_name}" ]]; then
      info "  (FOLDER_NAME='${folder_name}' is ignored because FOLDER_ID is set)"
    fi
